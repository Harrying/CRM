...
<body>
	<c:choose>
		<c:when test="${sessionScope.user.usr_role_id==4||sessionScope.user.usr_role_id==2}">
			...
			<form action="service/findByConFp?curpage=1" name="frm" method="post">
			...
			<table id="data_list_table" class="data_list_table" cellSpacing=1
				...
				<c:if test="${sessionScope.user.usr_role_id==4||sessionScope.user.usr_role_id==2}">
					<c:forEach items="${sessionScope.services}" var="cs">
						<tr>
							...
							<td><c:if
									test="${sessionScope.user.usr_name==cs.svr_due_to}">
									<a
										href="service/queryOneByid?svr_id=${cs.svr_id}">
										<img
										src="${pageContext.request.contextPath}/images/bt_deal.gif"
										class="op_button" />
									</a>
								</c:if></td>
						</tr>
				...
				<tr>
					<td colspan="8" align="center">共${sessionScope.pu.rows}条记录 每页
						6 条 第${sessionScope.pu.curPage}页 <a
						href="service/findAllFp?curpage=1">首页</a> <c:choose>
							<c:when test="${sessionScope.pu.curPage-1>0}">
								<a href="service/findAllFp?curpage=${sessionScope.pu.curPage-1}">上一页</a>
							</c:when>
							<c:otherwise>
							...
							<c:when test="${sessionScope.pu.pages-sessionScope.pu.curPage>0}">
								<a href="service/findAllFp?curpage=${sessionScope.pu.curPage+1}">下一页</a>
							</c:when>
						...
						</c:choose> <a href="service/findAllFp?curpage=${sessionScope.pu.pages}">尾页</a>
					</td>
				</tr>
			</table>
		</c:when>
